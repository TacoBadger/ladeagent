...
if [ ! -f /etc/nginx/sites-available/$DOMAIN ]; then
  cp deploy/nginx-fontlume.conf /etc/nginx/sites-available/$DOMAIN
...
if [ "$DNSIP" = "$MYIP" ]; then
  if [ ! -d /etc/letsencrypt/live/$DOMAIN ]; then
    certbot --nginx -d $DOMAIN -d www.$DOMAIN --non-interactive --agree-tos --redirect -m theispfrost@gmail.com
  fi
  echo "TLS: ok"
  curl -s -o /dev/null -w "public mcp initialize: HTTP %{http_code}\n" -X POST https://$DOMAIN/ladeagent/mcp \
    -H 'Content-Type: application/json' -H 'Accept: application/json, text/event-stream' \
    -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"curl","version":"0"}}}'
  echo "KLAR: https://$DOMAIN/ladeagent/mcp"
else
  echo "DNS for $DOMAIN peger på '$DNSIP', serveren er $MYIP. Ret A-record hos Hostinger til $MYIP og kør scriptet igen for TLS."
fi
